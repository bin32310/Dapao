...
<script type="text/javascript">
   ...
   function idCheck() {   
      if (!$('#us_id').val()) {
         ...
         $('#idConfirm').text('아이디(' + idLength + '자 제한)').css('color', 'black')
   ...
   function pwCheck(){
      if (!$('#us_pw').val()) {
         ...
         $('#pwConfirm').text('영문,숫자,특수기호 필수조건 입니다').css('color', 'black')
   ...
   function nickNameCheck() {
      if (!$('#us_nickname').val()) {
         ...
         $('#nickNameConfirm').text('닉네임(' + nickNameLength + '자 제한)').css('color', 'black')
   ...
   function emailCheck() {
      ...
      if (!email) {
         ...
         $('#emailConfirm').text('이메일 형식( @ 필수 )으로 적어주세요').css('color', 'red')
   ...
   <fieldset>
      <form action="" method="post" onsubmit="return check()">
         ...
         비밀번호<input   type="password" name="us_pw" id="us_pw" oninput="pwCheck()" pattern=".{3,}"
         ...
         닉네임<input type="text"name="us_nickname" id="us_nickname" oninput="nickNameCheck()" maxlength="8" required="required">
         	   <span id="nickNameConfirm" style="font-size: 12px; visibility:hidden;">닉네임을 입력하세요</span><br>
